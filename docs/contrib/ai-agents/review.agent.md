...
- _Détail technique :_ Explication brève de l'impact structurel.

## 4. Robustesse & "Edge Cases"

- **[Fichier : Ligne] :** [Type d'erreur potentielle, ex: IndexError, AttributeError]
  - _Contexte :_ Explication technique (ex: `my_list[0]` appelé sans vérification `if my_list`).
- **[Fichier : Ligne] :** [Type d'exception]
  - _Contexte :_ (ex: `try/except` générique masquant des bugs).
